...
export class Game {
    ...
    addLetter(char) {
        if (this._gameState !== GameState.PLAYING)
            return; // Si el juego no está en curso, no hace nada
        if (this._currentCol >= MAX_WORD_SIZE)
            return; // Si ya hay 5 letras, no hace nada
    ...
    removeLetter() {
        ...
        if (this._currentCol <= 0)
            return; // Si no hay letras, no hace nada
        this._currentCol--;
    ...
    submitWord() {
        if (this._gameState !== GameState.PLAYING)
            return; // Si el juego no está en curso, no hace nada
        if (this._currentCol < MAX_WORD_SIZE)
            return; // Si no hay 5 letras, no hace nadaº
